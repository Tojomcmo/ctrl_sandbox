...
def plot_2d_state_scatter_sequences(
    x_seq, xlabel="state 1", ylabel="state 2", title="state plot"
):
    """
...
def plot_2d_state_quiver(
    figure,
    axes,
    x_seq,
    legend_name="sequence",
    xlabel="state 1",
    ylabel="state 2",
    title="state plot",
    color="b.",
...
def plot_compare_state_sequences(
    plot_func,
    figure,
    axes,
    axis_num,
...
def plot_compare_state_sequences_quiver_dot(
    figure,
    axes,
    x_seqs,
    x_names,
    ...
    for idx in range(len(x_seqs)):
        plot_2d_state_quiver(
            ...
            ylabel,
            title,
            color=x_styles_quiver[idx],
            width=quiverwidth[idx],
        )
        plot_2d_state_dot(
            figure,
            axes,
            ...
            ylabel,
            title,
            color=x_styles_dot[idx],
        )
    plt.legend()
    return


def plot_x_y_sequences(
    figure,
    axes,
    x_seq,
    y_seq,
...
def plot_ilqr_iter_sim_ctrl_cost(
    ...
    x_plot_seqs.insert(0, controller_output.init_x_seq)
    x_plot_seqs.append(x_sim_seq)
    x_plot_seqs.append(x_des_seq)
    x_plot_seq_names = []
    ...
    plot_compare_state_sequences_quiver_dot(
        fig,
        ax1,
        x_plot_seqs,
        x_plot_seq_names,
    ...
    plot_x_y_sequences(
        fig,
        ax3,
        controller_output.time_seq,
        controller_output.cost_seq,
        xlabel="Time",
        ylabel="cost",
        datalabel="cost pred",
        title="cost plot",
    ...
    plot_x_y_sequences(
        ...
        ax3,
        controller_output.time_seq,
        controller_output.x_cost_seq,
        xlabel="Time",
        ylabel="cost",
        datalabel="state cost pred",
        title="cost plot",
        color="b.",
    )
    plot_x_y_sequences(
        ...
        ax3,
        controller_output.time_seq,
        controller_output.u_cost_seq,
        xlabel="Time",
        ylabel="cost",
        datalabel="control cost pred",
        title="cost plot",
        color="g.",
    )
    plt.tight_layout()
