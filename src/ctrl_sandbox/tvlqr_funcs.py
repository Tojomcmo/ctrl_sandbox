...
class ctrlOutput:
    def __init__(self, ctrl_config: ctrlConfig):
        ...
        self.k_fb_seq = np.zeros(
            [ctrl_config.len_seq - 1, ctrl_config.u_len, ctrl_config.x_len]
        )
        self.s_xx_seq = np.zeros(
            [ctrl_config.len_seq, ctrl_config.x_len, ctrl_config.x_len]
        )


def calc_tvlqr_controller(
    ctrl_config: ctrlConfig, A_seq, B_seq, x_nom_seq, u_nom_seq
) -> ctrlOutput:
    ...
    for idx in range(N - 1):
        ...
        s_xx_dt = gen_ctrl.calculate_s_xx_dot(
            ctrl_config.Q, A_seq[k], BRinvBT, ctrl_out.s_xx_seq[k + 1]
        )
        s_xx_km1 = ctrl_out.s_xx_seq[k + 1] - s_xx_dt * ctrl_config.time_step
        ctrl_out.s_xx_seq[k] = s_xx_km1
        ctrl_out.k_fb_seq[k] = calculate_k_fb(
            ctrl_config.R, B_seq[k], ctrl_out.s_xx_seq[k]
        )
    return ctrl_out
...
def simulate_lti_tvlqr(
    ...
    A_sim_cont_seq = np.zeros(
        [ctrl_config.len_seq - 1, A_sim_cont.shape[0], A_sim_cont.shape[1]]
    )
    B_sim_cont_seq = np.zeros(
        [ctrl_config.len_seq - 1, B_sim_cont.shape[0], B_sim_cont.shape[1]]
    )
    ...
    for k in range(ctrl_config.len_seq - 1):
        u_sim_seq[k] = calculate_u_star(
            ctrl_out.k_fb_seq[k],
            ctrl_out.u_nom_seq[k],
            ctrl_out.x_nom_seq[k],
            x_sim_seq[k],
        )
        x_dyn_func_k = lambda x_nom, u_nom: A_sim_cont @ x_nom + B_sim_cont @ u_nom
        x_sim_seq[k + 1] = gen_ctrl.simulate_forward_dynamics_step(
            x_dyn_func_k,
            x_sim_seq[k],
            u_sim_seq[k],
...
def plot_2d_state_and_control(
    x_nom_seq, ctrl_out, x_sim_seq, u_sim_seq, time_seq, ax1, ax2
):
    ax1.plot(
        x_nom_seq[0, 0],
        x_nom_seq[0, 1],
    ...
    ax1.plot(
        x_sim_seq[0, 0], x_sim_seq[0, 1], color="red", marker="o", label="sim start"
    )
    ...
    ax1.legend()
    ax2.plot(
        time_seq, ctrl_out.u_nom_seq[:, 0], color="blue", label="nominal control input"
    )
    ax2.plot(
        time_seq,
        u_sim_seq[:, 0],
        color="red",
        label="optimal control input",
        linestyle="--",
    )
    ax2.legend()
