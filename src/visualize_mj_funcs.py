...
def create_mj_video_ilqr_w_ctrl(model, 
                                data, 
                                renderer, 
                                scene_option, 
                                framerate:int, 
                                x_init, 
                                ctrl_out:ilqr.ilqrControllerState, ilqr_config:ilqr.ilqrConfigStruct):
    '''
...
def create_mj_video_w_ctrl_with_different_time_steps(model, data, renderer, scene_option, framerate, x_init, ctrl_out, ilqr_config):
    '''
        function simulates provided model and data forward in time using u sequence
        and returns state trajectory x_seq and linearized state and control matricies A
        and B at each pair x_seq, u_seq
