...
def sim_dyn(discrete_dyn_func:Callable[[int, jnp.ndarray, jnp.ndarray],jnp.ndarray],
                 ...
                 disturb_func:Callable[[int, jnp.ndarray], jnp.ndarray],
                   noise_func:Callable[[int, jnp.ndarray], jnp.ndarray],
                       x_init:jnp.ndarray,
                      seq_len:int)->Tuple[jnp.ndarray,jnp.ndarray, jnp.ndarray]:
    seq = jnp.arange(0, seq_len-1,dtype=int)
    carry_init = (x_init, x_init)
    scan_func = curry_sim_dyn_scan_func(discrete_dyn_func,control_func, measure_func, disturb_func, noise_func)
...
def prep_disturb_noise_WGN_for_sim(process_cov_mat:jnp.ndarray, 
                                   noise_cov_mat:jnp.ndarray)->Tuple[Callable[[int, jnp.ndarray],jnp.ndarray],
...
def prep_ff_fb_u_for_sim(u_ff_seq:jnp.ndarray, 
                         x_des_seq:jnp.ndarray, 
                         k_fb_seq:jnp.ndarray)->Callable[[int, jnp.ndarray], jnp.ndarray]:
