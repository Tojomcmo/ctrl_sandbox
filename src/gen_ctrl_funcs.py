...
class stateSpace:
    def __init__(self, a_mat:jnp.ndarray, b_mat:jnp.ndarray, c_mat:jnp.ndarray, d_mat:jnp.ndarray, time_step = None):
...
def _dyn_for_sim_scan_func(
        ...
        carry_x_k:jnp.ndarray, u_seq:jnp.ndarray)->Tuple[jnp.ndarray, jnp.ndarray]:
    '''
...
def _curry_dyn_for_sim_scan_func(
        discrete_dyn_func:Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray])->Callable[[jnp.ndarray,jnp.ndarray],
                                                                                  Tuple[jnp.ndarray,jnp.ndarray]]:
    def scan_func(carry_x_k:jnp.ndarray, u_seq:jnp.ndarray)->Tuple[jnp.ndarray, jnp.ndarray]:
        return _dyn_for_sim_scan_func(discrete_dyn_func,carry_x_k,u_seq)
    return scan_func

def simulate_forward_dynamics_seq_pre_decorated(
        sim_dyn_scan_func:Callable[[jnp.ndarray,jnp.ndarray],Tuple[jnp.ndarray,jnp.ndarray]], 
        x_init:jnp.ndarray, u_seq:jnp.ndarray) -> jnp.ndarray:
...
def simulate_forward_dynamics_seq(
    ...
    '''**Forward simulate a control sequence from an initial state through a discrete dynamics function** \n
        ...
        - return x_seq - [out] state sequence, dimension(len_seq,n)
    '''
    discrete_dyn_func_scan = lambda carry_x_k, u_seq: _dyn_for_sim_scan_func(discrete_dyn_func, carry_x_k, u_seq)
...
def _dyn_lin_scan_func(
        dyn_func:Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray],
        x_len:int, carry:None, x_and_u_k:jnp.ndarray) -> Tuple[None, Tuple[jnp.ndarray, jnp.ndarray]]:
    '''
...
def _curry_dyn_lin_scan_func(
        dyn_func:Callable[[jnp.ndarray, jnp.ndarray], jnp.ndarray], 
        x_len:int)->Callable[[None, jnp.ndarray],Tuple[None, Tuple[jnp.ndarray, jnp.ndarray]]]:
    '''
...
def calculate_linearized_state_space_seq_pre_decorated(
        dyn_lin_scan_func:Callable[[None, jnp.ndarray],Tuple[None, Tuple[jnp.ndarray, jnp.ndarray]]], 
        x_seq:jnp.ndarray, u_seq:jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
    '''
...
def step_euler_forward(
        ...
        h:float, x_k:jnp.ndarray, u_k:jnp.ndarray) -> jnp.ndarray:
...
def step_solve_ivp(
        ...
        h:float, x_k:npt.NDArray[np.float64], u_k:npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
...
def _cost_for_calc_scan_func(
        ...
        x_len:int, carry:Tuple[int, jnp.ndarray], xu_k)-> Tuple[Tuple[int,jnp.ndarray], 
...
def _curry_cost_for_calc_scan_func(
    cost_func:Callable[[jnp.ndarray, jnp.ndarray, int], Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]],
    x_len:int)->Callable[[Tuple[int, jnp.ndarray],jnp.ndarray],
...
def calculate_total_cost_pre_decorated(
        cost_for_calc_scan_func:Callable[[Tuple[int, jnp.ndarray],jnp.ndarray],
                                    Tuple[Tuple[int, jnp.ndarray],Tuple[jnp.ndarray,jnp.ndarray,jnp.ndarray]]], 
        x_seq:jnp.ndarray, 
    ...
    **Calculate the x and u sequence cost using a pre-decorated cost function for lax.scan**
    ...
    (_, total_cost), (total_cost_seq, x_cost_seq, u_cost_seq) = lax.scan(cost_for_calc_scan_func, 
                                                                         init_carry,xu_seq)
...
def calculate_total_cost(
    ...
    **Calculate the x and u sequence cost from provided undecorated cost function**
    ...
    xu_seq                             = _combine_x_u_for_cost_scan(x_seq, u_seq)
    init_carry:Tuple[int, jnp.ndarray] = (0, jnp.zeros([1], dtype=float))
    scan_cost_func_for_calc_curried    = _curry_cost_for_calc_scan_func(cost_func,x_seq.shape[1])
    (_, total_cost), (total_cost_seq, x_cost_seq, u_cost_seq) = lax.scan(scan_cost_func_for_calc_curried, 
                                                                         init_carry,xu_seq)
    return total_cost.item(), total_cost_seq, x_cost_seq, u_cost_seq
...
def taylor_expand_cost(
        ...
        xu_k:jnp.ndarray,
        x_k_len:int, k:int)-> Tuple[jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray]:
    ''' 
...
def _curry_cost_for_exp_scan_func(
        cost_func:Callable[[jnp.ndarray, jnp.ndarray, int], jnp.ndarray], 
        x_len:int)->Callable[[int, jnp.ndarray],Tuple[int, Tuple[jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray]]]:
    def scan_func(carry:int, xu_k:jnp.ndarray)->Tuple[int,Tuple[jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray]]:
        return _cost_for_exp_scan_func(cost_func,x_len,carry, xu_k)
    return scan_func

def taylor_expand_cost_seq_pre_decorated(
        cost_for_exp_scan_func: Callable[[int, jnp.ndarray],Tuple[int, Tuple[jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray,jnp.ndarray]]],
        x_seq:jnp.ndarray, u_seq:jnp.ndarray)->Tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    u_seq_end = jnp.zeros((1,u_seq.shape[1]), dtype=float)
    u_seq_mod = jnp.append(u_seq, u_seq_end, axis=0)
...
def taylor_expand_cost_seq(
    ...
    xu_seq    = jnp.concatenate([x_seq, u_seq_mod], axis=1)
    carry:int = 0
    lin_scan_func_curried = lambda carry, xu_seq: _cost_for_exp_scan_func(cost_func, x_seq.shape[1], carry, xu_seq)
...
def prep_cost_func_for_diff(
    cost_func:Callable[[jnp.ndarray, jnp.ndarray, int],
...
def calculate_ff_fb_u(
        k_fb_k:jnp.ndarray,u_ff_k:jnp.ndarray, 
        x_des_k:jnp.ndarray,x_k:jnp.ndarray) -> jnp.ndarray:
    return (u_ff_k.reshape(-1,1) + k_fb_k @ (x_k.reshape(-1,1) - x_des_k.reshape(-1,1))).reshape(-1)

def apply_cov_noise_to_vec(
        vec:jnp.ndarray, cov_mat:jnp.ndarray)->jnp.ndarray:
    return vec + jax.random.multivariate_normal(cov_mat.shape[0],np.zeros(cov_mat.shape[0]),cov_mat)

######################################################################################################

def ss_2_dyn_func(ss_cont:stateSpace)->Tuple[Callable[[jnp.ndarray,jnp.ndarray],jnp.ndarray],bool]:
    if ss_cont.type == 'continuous':
        is_continous_bool = True
    else:
        is_continous_bool = False
    return (lambda x, u: ss_cont.a @ x.reshape(-1,1) + ss_cont.b @ u.reshape(-1,1)), is_continous_bool
...
def convert_d2d_A_B(
        Ad_1:jnp.ndarray, Bd_1:jnp.ndarray, dt_1:float, dt_2:float)->Tuple[jnp.ndarray, jnp.ndarray]:
...
def calculate_matrix_power_similarity_transform(A:jnp.ndarray, exponent:float)-> jnp.ndarray:
...
def discretize_continuous_state_space(
        input_state_space:stateSpace, time_step:float, c2d_method:str = 'euler') -> stateSpace:
    '''
    **Function for discretizing continuous state space with different discretization methods** \n
    ...
    if input_state_space.type == 'discrete':
        ...
        #   create combined A B matrix e^([[A, B],[0,0]]
            a_b_c:jnp.ndarray  = jnp.concatenate((jnp.concatenate((input_state_space.a, 
                                                                   input_state_space.b),
                                                                   axis=1),jnp.zeros((m,n + m))), axis=0)
