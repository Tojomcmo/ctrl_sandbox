...
def _curry_dyn_for_sim_scan_func(
    ...
    """
    **Create lax.scan compatible function for forward dynamic simulation through partial argument reduction**
    """
...
def simulate_forward_dynamics_seq(
    ...
    """
    **Simulate forward dynamics from initial state through control sequence with undecorated dynamics function** \n
...
def calculate_linearized_state_space_seq(
    ...
    """
    **Calculate the A and B matrix linearizations for x and u through undecorated dynamics function** \n
    - Wrapper function for calculate_linearized_state_space_seq_pre_decorated().
    - Simpler interface by accepting dynamics function directly
    - Slower in iteration due to the new function definition on each call.
    """
    lin_scan_func = _curry_dyn_lin_scan_func(dyn_func, x_seq.shape[1])
    return calculate_linearized_state_space_seq_pre_decorated(
        lin_scan_func, x_seq, u_seq
    )
...
def _cost_for_calc_scan_func(
    ...
    """
    ** Kernel function for lax.scan calculation of sequence cost and individual cost sequences**
    - [in] cost_func - callable cost function that accepts (x_k, u_k, k) and returns the total and individual costs as jnp.ndarrays of dim(1,)
    - [in] x_len - length of x_vector, dim n (not sequence length len_seq)
    - [in] carry - Tuple of sequence index and cost sum, serves as carry argument for lax.scan
    - [in] xu_k - combined x and u sequences in jnp.ndarray dim(len_seq, n+m), serves as seq argument for lax.scan
    """
    k, cost_sum = carry
...
def _curry_cost_for_calc_scan_func(
    ...
    """
    **Create lax.scan compatible function for cost calculation through partial argument reduction**
    """
...
def _cost_for_exp_scan_func(
    ...
    ** kernel lax.scan function for cost function second order taylor expansion**
    - [in] cost_func - callable that accepts (x_k, u_k, k) and returns cost as a jnp.ndarray of dim(1,) type float
    - [in] x_len - length of x vector
    - [in] k - int index of sequence, serves as carry variable in lax.scan
    - [in] xu_k - paired x and u sequences as jnp.ndarray of dim (len_seq, n+m), u_seq[-1] must be vector of zeros
    - [out] kp1 - sequence index incremented by 1
    - [out] (expanded_mats) - taylor expanded matrices around xu_k primal point. refer to taylor_expand_cost() for details
    """
...
def _curry_cost_for_exp_scan_func(
    ...
    """
    **Create lax.scan compatible function for cost function taylor expansion through partial argument reduction**
    """
...
def taylor_expand_cost_seq_pre_decorated(
    ...
    """
    **Calculate the second order taylor expansion provided undecorated cost function at x and u sequence primal points** \n
    """
    u_seq_end = jnp.zeros((1, u_seq.shape[1]), dtype=float)
    u_seq_mod = jnp.append(u_seq, u_seq_end, axis=0)
...
def calculate_ff_fb_u(
    ...
    **calculate control value at discrete time k from feedforward-feedback structure**
    - [in] k_fb_k - feedback gain matrix at time k, dim(m,n) providing control interaction with state error
    - [in] u_ff_k - feedforward/nominal control value at time k, prescribed trajectory control sequence
    - [in] x_des_k - desired/target state trajectory at time k, prescribed desired state
    - [in] x_k - actual/measured/estimated state at time k, differenced with x_des_k to create vectored error value
