...
if __name__ == "__main__":
    ...
    dt = 0.005
    len_seq = 1000
    time_vec = jnp.arange(0, len_seq * dt, dt)
    h_bar = 1.0
    r_bar = 0.05
    m_bar = 1.0
    d_bar = h_bar / 2
    moi = (1 / 12) * h_bar * (m_bar**2 + 3 * r_bar**2)
    dpend_sys = dyn.double_pend_rel_dyn(
        g=9.81,
        m1=m_bar,
        moi1=moi,
        d1=d_bar,
    ...
    x_init = jnp.array([1.0, 1.0, 1.0, 1.0])
    ...
    for k in range(len_seq - 1):
        x_seq = x_seq.at[k + 1].set(
            integrate.step_rk4(dpend_sys.cont_dyn_func, dt, x_seq[k], u_vec)
        )
    ...
    pend_animation = vis_dyn.double_pm_pend_animation(
        dpend_sys.l1, dpend_sys.l2, np.array(x_seq), dt, fig, th2="rel"
    )
