...
class undamped_simple_pend_dyn():
    ...
    def cont_dyn_func(self, state:np.ndarray, control:np.ndarray)->np.ndarray:
        ...
        state_dot = np.array([
                    state[1],
                    - (np.sin(state[0]) * self.g/self.l) + control[0]
                    ])
        return state_dot    
...
if __name__=="__main__":
    ...
    mujoco.mjd_transitionFD(mj_model, mj_data, eps, flg_centered, A_mj, B_mj, None, None)
    ...
    x_kp1_pred_lin = ss_disc.A @ state_k.reshape(-1,1) + ss_disc.B @ control_k.reshape(-1,1)
