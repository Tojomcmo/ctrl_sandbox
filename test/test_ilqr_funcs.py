...
class shared_unit_test_data_and_funcs:
    def __init__(self) -> None:
        ...
        self.K_seq_example = np.ones(
            [self.ilqr_config.len_seq, self.ilqr_config.u_len, self.ilqr_config.x_len]
        )
        self.d_seq_example = np.ones(
            [self.ilqr_config.len_seq, self.ilqr_config.u_len, 1]
        )
    ...
    def pend_dyn_nl(self, params, state, control):
        ...
        state_dot = jnp.array(
            [
                state[1],
                -(params.b / params.l) * state[1]
                - (jnp.sin(state[0]) * params.g / params.l)
                + control[0],
            ]
        )
        return state_dot
    ...
    def pend_dyn_nl_func_unit_param_curried(self, state, control):
        ...
        vel = state[1]
        tq = control[0]
        state_dot = jnp.array([vel, -(b / l) * vel - (jnp.sin(pos) * g / l) + tq])
        return state_dot
    ...
    def cost_func_quad_state_and_control(
        ...
        if is_final_bool:
            x_k_corr = util.calc_and_shape_array_diff(
                x_k, cost_func_params.x_des_seq[k_step], shape="col"
            )
            x_cost = jnp.array((0.5) * (jnp.transpose(x_k_corr) @ Qf @ x_k_corr))
            u_cost = jnp.array([0.0])
            total_cost = x_cost
        elif k_step == 0:
            u_k_corr = util.calc_and_shape_array_diff(
                u_k, cost_func_params.u_des_seq[k_step], shape="col"
            )
    ...
    def cost_func_quad_state_and_control_unit_param_curried(
        ...
        cost_func_curried = lambda x, u, k, is_final_bool=False: cost_func(
            cost_func_params, x, u, k, x_des_seq, u_des_seq, is_final_bool
        )
...
class initialize_ilqr_controller_tests(unittest.TestCase):
    ...
    def test_accepts_valid_inputs(self):
        ...
        x_seq, cost_float, prev_cost_float, cost_seq = ilqr.initialize_ilqr_controller(
            ilqr_config, ctrl_state
        )
        self.assertEqual(
            (np.abs(cost_float - prev_cost_float) > ilqr_config.converge_crit), True
        )
...
class run_ilqr_controller_tests(unittest.TestCase):
    def test_accepts_valid_linear_inputs(self):
        ...
        state_seq, cost_float, prev_cost_float, cost_seq = (
            ilqr.initialize_ilqr_controller(ilqr_config, ilqr_ctrl_state)
        )
...
class calculate_backwards_pass_tests(unittest.TestCase):
    def test_accepts_valid_linear_dyn_inputs(self):
        ...
        k_seq, d_seq, Del_V_vec_seq, ro_reg = ilqr.calculate_backwards_pass(
            data_and_funcs.ilqr_config, data_and_funcs.ilqr_ctrl_state
        )
        ...
        self.assertEqual(
            jnp.shape(k_seq[0]),
            (data_and_funcs.ilqr_config.u_len, data_and_funcs.ilqr_config.x_len),
        )
...
class calculate_forwards_pass_tests(unittest.TestCase):
    def test_accepts_valid_linear_dyn_inputs(self):
        ...
        (
            ...
            line_search_factor,
            ro_reg_change_bool,
        ) = ilqr.calculate_forwards_pass(ilqr_config, ilqr_ctrl_state)
        self.assertEqual(True, True)


class simulate_forwards_pass_tests(unittest.TestCase):
    def example_cost_func(
    ...
    def test_accepts_valid_system(self):
        ...
        fp_scan_func_curried = (
            lambda lsf, carry_in, seqs_in: ilqr.forward_pass_scan_func(
                self.example_dyn_func, self.example_cost_func, lsf, carry_in, seqs_in
            )
        )
        x_seq_new, u_seq_new, cost_float = ilqr.simulate_forward_pass(
            fp_scan_func_curried,
            self.example_cost_func,
            x_seq,
            u_seq,
            K_seq,
            d_seq,
...
class scan_func_forward_pass_tests(unittest.TestCase):
    ...
    def test_accepts_valid_system(self):
        ...
        u_k_new_expected = (
            u_k.reshape(-1, 1)
            + K_k @ (x_k.reshape(-1, 1) - x_k.reshape(-1, 1))
            + lsf * d_k
        ).reshape(-1)
...
class calculate_optimal_gains_tests(unittest.TestCase):
    def test_accepts_valid_inputs(self):
        ...
        d_k_expected = -(jnp.array([[1.0, -1.0], [0, 1.0]])) @ (
            jnp.array([[1.0], [1.0]])
        )
...
class calculate_u_k_new_tests(unittest.TestCase):
    ...
    def test_reject_invalid_K_k_dimension(self):
        ...
        self.assertEqual(
            str(assert_seq_len_error.exception),
    ...
    def test_reject_invalid_d_k_dimension(self):
        ...
        self.assertEqual(
            str(assert_seq_len_error.exception),
            "d_k incorrect shape, must be (len(u_nom_k), 1)",
        )
    ...
    def test_reject_invalid_x_dimension(self):
        ...
        self.assertEqual(
            str(assert_seq_len_error.exception), "x_nom_k must be column vector (n,)"
        )
...
class calculate_cost_decrease_ratio_tests(unittest.TestCase):
    ...
    def test_rejects_invalid_input_dimension(self):
        ...
        self.assertEqual(
            str(assert_seq_len_error.exception),
            "Del_V_vec_seq is not the right shape, must be (:,2)",
        )
...
class calculate_expected_cost_decrease_tests(unittest.TestCase):
    def test_accepts_valid_inputs(self):
        ...
        Del_V_vec_seq[:, 1] *= 2.0
        line_search_factor = 2.0
        Del_V_sum = ilqr.calculate_expected_cost_decrease(
            Del_V_vec_seq, line_search_factor
        )
